...
function normalizeDiagnosticQuestion(question = "") {
  return String(question)
    .replace(/^这是某位候选人在[^：:]+(?:被追问的版本|被问到的)?[：:]\s*/u, "")
    .replace(/^系统生成诊断题[：:]\s*/u, "")
    .trim();
}
        ...
        items: [
          {
            ...
            evidenceSnippet: "高频追问集中在 AQS、ReentrantLock、state、队列变化与唤醒语义的关系。",
            misconception: "容易只背 AQS 是同步器框架，却讲不清 acquire/release 主链路。",
            importance: "core",
            diagnosticQuestion:
              "如果 ReentrantLock 建在 AQS 之上，独占 acquire/release 这条主链路你会怎么解释？",
          ...
          },
          {
            ...
            evidenceSnippet: "高频追问是：为什么线程写了共享变量，另一个线程不一定立刻看到，volatile 到底改变了什么。",
            misconception: "容易只背结论，不会把可见性问题和具体同步原语联系起来。",
            importance: "core",
            diagnosticQuestion:
              "为什么一个线程写了共享变量，另一个线程不一定立刻看到？volatile 到底改变了什么？",
          ...
          },
          {
            ...
            evidenceSnippet: "真实面试会追问线程池为什么这么配、满了会怎样、为什么不能共享一个线程池。",
            misconception: "容易只背线程池 7 个参数，却说不清不同业务为什么不能共用同一个线程池。",
            importance: "secondary",
            diagnosticQuestion: "为什么线上不同业务不能随便共用一个线程池？线程池参数到底是在控制什么？",
        ...
        items: [
          {
            ...
            evidenceSnippet: "面试常追问 RR 为什么还要 next-key lock，以及快照读 / 当前读边界。",
            misconception: "容易把 MVCC、锁和隔离级别混成一团，只背结论不说边界。",
            importance: "core",
            diagnosticQuestion:
              "MySQL Repeatable Read 已经有 MVCC 了，为什么还会谈 next-key lock？",
          ...
          },
          {
            ...
            evidenceSnippet: "索引相关追问常落在 B+ 树、最左前缀、覆盖索引、索引失效和执行计划理解上。",
            misconception: "容易只背『索引能加速查询』，不会解释为什么优化器有时仍然不走索引。",
            importance: "core",
            diagnosticQuestion: "一个查询为什么明明建了索引，执行计划里还是没用上？",
          ...
          },
          {
            ...
            evidenceSnippet: "高频问题不是背名字，而是讲清楚谁负责崩溃恢复、谁负责回滚、谁负责主从复制。",
            misconception: "容易只记得三大日志名字，却说不清为什么它们不能相互替代。",
            importance: "core",
            diagnosticQuestion: "MySQL 里的 redo log、undo log、binlog 分别在管什么？为什么不能只有一种日志？",
        ...
        items: [
          {
            ...
            evidenceSnippet: "后端面经里常把缓存策略和数据库更新顺序放在一起追问。",
            misconception: "容易只背套路，不会说为什么这个顺序仍有脏读窗口。",
            importance: "core",
            diagnosticQuestion:
              "更新数据库和删除 Redis 缓存时，为什么顺序不对会出一致性问题？",
          ...
          },
          {
            ...
            evidenceSnippet: "高频追问会把 RDB/AOF、阻塞、大 key/hot key 和线上恢复放在一起考。",
            misconception: "容易只背 Redis 快，却讲不清持久化和阻塞为什么会影响线上表现。",
            importance: "core",
            diagnosticQuestion: "Redis 的 RDB 和 AOF 分别怎么取舍？为什么线上还会担心阻塞和数据丢失？",
        ...
        items: [
          {
            ...
            evidenceSnippet: "服务链路面经常追问 @Transactional 为什么有时候不生效。",
            misconception: "容易只记住自调用会失效，但讲不清本质是代理边界。",
            importance: "core",
            diagnosticQuestion:
              "为什么同一个类里自调用一个标了 @Transactional 的方法，事务可能不生效？",
          ...
          },
          {
            ...
            evidenceSnippet: "这类题核心不是背 IoC 和 AOP 定义，而是把代理对象如何接住业务调用、如何串起通知和目标方法讲清楚。",
          ...
          },
          {
            ...
            evidenceSnippet: "高频追问会从 @SpringBootApplication、条件装配和 starter 机制切入。",
        ...
        items: [
          {
            ...
            evidenceSnippet: "高频场景题会把超时、重试风暴、幂等约束和补偿一起问。",
          ...
          },
          {
            ...
            evidenceSnippet: "场景题通常不是问定义，而是问在高压链路里你会怎么保核心路径。",
        ...
        items: [
          {
            ...
            evidenceSnippet: "真实面试常从 Partition、Consumer Group、ISR、Rebalance 切入。",
          ...
          },
          {
            ...
            evidenceSnippet: "实际场景题更关心消息丢失、乱序、重复消费和业务补偿如何平衡。",
        ...
        items: [
          {
            ...
            evidenceSnippet: "高频问题集中在堆、栈、方法区、年轻代/老年代和常见回收路径。",
          ...
          },
          {
            ...
            evidenceSnippet: "面试里常问：类是怎么被加载的，双亲委派为什么重要，又在哪些场景会打破。",
        ...
        items: [
          {
            ...
            evidenceSnippet: "高频问题不只问三次握手，还会追问半连接队列、全连接队列、TIME_WAIT 和 overflow。",
          ...
          },
          {
            ...
            evidenceSnippet: "高频追问往往不是死背状态码，而是问哪些状态说明了重试、缓存或接口语义问题。",
        ...
        items: [
          {
            ...
            evidenceSnippet: "通用 Java 并发面里依然常见。",
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            evidenceSnippet: "这是通用后端数据库面也经常出现的链路问题。",
        ...
        items: [
          {
            ...
            evidenceSnippet: "通用后端面经里经常追问事务为什么不生效。",
        ...
        items: [
          {
            ...
            title: "TCP 三次握手 / 四次挥手 / backlog / TIME_WAIT",
            summary: "能解释 TCP 建连与断连关键状态，以及 backlog/TIME_WAIT 的服务端含义。",
            evidenceSnippet: "这是通用后端面里最常见的网络高频追问之一。",
