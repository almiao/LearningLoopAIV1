...
//
// 挂在现有 practice 回路上：每完成一轮 practice，额外判一次一次性锚点，把「漏掉的」写成
// 复习项进失败账本。纪律：
//   - 只写不读：不读老 store、不改 UI、不做数据迁移。
//   - 失败驱动：只有判 fail 且有漏点才写（§4「练过不留痕」）。
//   - 绝不拖垮主流程：判分/写盘任何报错都吞掉，practice 照常进行（旁路、尽力而为）。
...
export function createPracticePassthrough({ store, judge, logger = console } = {}) {
  ...
  return async function recordPracticeRound({
    ...
    try {
      ...
      // 旁路、尽力而为：写账本失败绝不能影响用户正在进行的 practice。
      const message = error && error.message ? error.message : String(error);
      if (logger && typeof logger.warn === "function") {
        logger.warn(`[ledger] practice passthrough skipped: ${message}`);
      }
      return null;
    }
  };
