...
verify_pid_files() {
  ...
  for service in ai-service bff superapp-service livekit-agent frontend; do
...
- LiveKit agent bridge: http://127.0.0.1:${LIVEKIT_AGENT_PORT}
...
(
  ...
  NEXT_PUBLIC_INTERVIEW_ASSIST_API_BASE_URL="http://127.0.0.1:${LIVEKIT_AGENT_PORT}" \
...
nohup bash -lc "
  cd '$ROOT_DIR'
  exec env PORT='$LIVEKIT_AGENT_PORT' AI_SERVICE_URL='http://127.0.0.1:${AI_PORT}' npm run start --prefix livekit-agent
...
nohup bash -lc "
  cd '$ROOT_DIR/frontend'
  exec env PORT='$FRONTEND_PORT' NEXT_PUBLIC_API_BASE_URL='http://127.0.0.1:${BFF_PORT}' NEXT_PUBLIC_INTERVIEW_ASSIST_API_BASE_URL='http://127.0.0.1:${LIVEKIT_AGENT_PORT}' npm run start -- --port '$FRONTEND_PORT'
